...
const AP_Param::GroupInfo AP_MotorsHeli_Compound::var_info[] PROGMEM = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("GYR_GAIN", 6, AP_MotorsHeli_Compound, _ext_gyro_gain, AP_MOTORS_HELI_COMPOUND_EXT_GYRO_GAIN),
...
void AP_MotorsHeli_Compound::enable()
{
    ...
    hal.rcout->enable_ch(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_4]));    // yaw_1
    hal.rcout->enable_ch(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_5]));    // yaw_2
    hal.rcout->enable_ch(AP_MOTORS_HELI_COMPOUND_AUX_1);                                 // output for gyro gain or direct drive variable pitch tail motor
    hal.rcout->enable_ch(AP_MOTORS_HELI_COMPOUND_AUX_2);                                 // output for gyro gain or direct drive variable pitch tail motor
    hal.rcout->enable_ch(AP_MOTORS_HELI_COMPOUND_RSC);                                 // output for main rotor esc

    // disable channels 7 and 8 from being used by RC_Channel_aux
    RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_HELI_COMPOUND_AUX_1]);
    RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_HELI_COMPOUND_AUX_2]);
    RC_Channel_aux::disable_aux_channel(_motor_to_channel_map[AP_MOTORS_HELI_COMPOUND_RSC]);
}
...
void AP_MotorsHeli_Compound::output_test(uint8_t motor_seq, int16_t pwm)
{
    ...
    switch (motor_seq) {
        ...
        case 5:
            // main rotor
            hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_HELI_COMPOUND_RSC]), pwm);
            break;
        default:
            // do nothing
            break;
...
bool AP_MotorsHeli_Compound::allow_arming() const
{
...
void AP_MotorsHeli_Compound::set_desired_rotor_speed(int16_t desired_speed)
{
    _main_rotor.set_desired_speed(desired_speed);

    // always send desired speed to tail rotor control, will do nothing if not DDVPT not enabled
    _tail_rotor_1.set_desired_speed(_direct_drive_tailspeed);
    _tail_rotor_2.set_desired_speed(_direct_drive_tailspeed);
}
...
void AP_MotorsHeli_Compound::calculate_scalars()
{
    ...
    if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH) {
        _tail_rotor_1.set_control_mode(AP_MOTORS_HELI_RSC_MODE_SETPOINT);
        _tail_rotor_1.set_ramp_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RAMP_TIME);
        _tail_rotor_1.set_runup_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RUNUP_TIME);
        _tail_rotor_1.set_critical_speed(_rsc_critical);
        _tail_rotor_1.set_idle_output(_rsc_idle_output);
        _tail_rotor_2.set_control_mode(AP_MOTORS_HELI_RSC_MODE_SETPOINT);
        _tail_rotor_2.set_ramp_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RAMP_TIME);
        _tail_rotor_2.set_runup_time(AP_MOTORS_HELI_COMPOUND_DDVPT_RUNUP_TIME);
        _tail_rotor_2.set_critical_speed(_rsc_critical);
        _tail_rotor_2.set_idle_output(_rsc_idle_output);
    } else {
        _tail_rotor_1.set_control_mode(AP_MOTORS_HELI_RSC_MODE_DISABLED);
        _tail_rotor_1.set_ramp_time(0);
        _tail_rotor_1.set_runup_time(0);
        _tail_rotor_1.set_critical_speed(0);
        _tail_rotor_1.set_idle_output(0);
        _tail_rotor_2.set_control_mode(AP_MOTORS_HELI_RSC_MODE_DISABLED);
        _tail_rotor_2.set_ramp_time(0);
        _tail_rotor_2.set_runup_time(0);
        _tail_rotor_2.set_critical_speed(0);
        _tail_rotor_2.set_idle_output(0);
    }
    _tail_rotor_1.recalc_scalers();
    _tail_rotor_2.recalc_scalers();
}
...
uint16_t AP_MotorsHeli_Compound::get_motor_mask()
{
    ...
    return (1U << 0 | 1U << 1 | 1U << 2 | 1U << 3 | 1U << 4 | 1U << AP_MOTORS_HELI_COMPOUND_AUX_1 | 1U << AP_MOTORS_HELI_COMPOUND_AUX_2 | 1U << AP_MOTORS_HELI_COMPOUND_RSC);
}

// update_motor_controls - sends commands to motor controllers
void AP_MotorsHeli_Compound::update_motor_control(AP_MotorsHeli_RSC::MotorControlState state)
{
    // Send state update to motors
    _tail_rotor_1.output(state);
    _tail_rotor_2.output(state);
    ...
    _heliflags.rotor_runup_complete = ( _main_rotor.is_runup_complete() && _tail_rotor_1.is_runup_complete() && _tail_rotor_2.is_runup_complete() );
}

// set_delta_phase_angle for setting variable phase angle compensation and force
// recalculation of collective factors
void AP_MotorsHeli_Compound::set_delta_phase_angle(int16_t angle)
{
...
//                       yaw:   -4500 ~ 4500
//
void AP_MotorsHeli_Compound::move_actuators(int16_t roll_out, int16_t pitch_out, int16_t coll_in, int16_t yaw_out)
{
    ...
    if (_servo_mode == SERVO_CONTROL_MODE_AUTOMATED) {
        ...
        if ((_main_rotor.get_control_output() > _rsc_idle_output) && _tail_type != AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO_EXTGYRO) {
...
void AP_MotorsHeli_Compound::move_yaw(int16_t yaw_in)
{
  ...
  if (_tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_SERVO || _tail_type == AP_MOTORS_HELI_COMPOUND_TAILTYPE_DIRECTDRIVE_VARPITCH) {
...
//    int16_t boost_out=4500*_boost_in;
    ...
    hal.rcout->write(pgm_read_byte(&_motor_to_channel_map[AP_MOTORS_MOT_4]), _yaw_servo_1.radio_out);
...
bool AP_MotorsHeli_Compound::parameter_check() const
{
